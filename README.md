...
처음에는 세션 스토리지를 사용하여 스크롤 위치와 불러왔던 상품 개수를 다시 읽어 뛰우는 형태였지만  
세션 스토리지가 초기화 되더라도 history 이벤트로 페이지 호출 시 항상 이전 값을 불러오기 위해  
...
이를 해결하기위해 검색을 통해 history.pushState()를 알게 되었고  
상품 페이지를 가기 전에 history를 한번 기록하고 넘어가며 보고 있던 정보들을 state를 활용하여 기록 하게 되면서  
해결하게 되었습니다.  
...
모든 페이지 전환에 대한 함수를 따로 만들기에는 비효율 적이라 생각되었고 이 문제는 소 메뉴 별로 상품 정보를 가져올 때마다 
history에 기록하면 될 것으로 생각합니다. 다만 이 경우에는 히스토리가 쓸데없이 누적된다는 문제점이 있을 것 같아 그냥 두도록 했습니다.  
