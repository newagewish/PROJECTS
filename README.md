...
처음에는 세션 스토리지를 사용하여 스크롤 위치와 불러왔던 상품 개수를 다시 읽어 뛰우는 형태였지만 세션 스토리지가 초기화 되더라도 history 이벤트로 페이지 호출 시 항상 이전 값을 불러오기 위해  
세션 스토리지 값을 history.state에 넣어서 사용하도록 했습니다.  

history.state 사용 이유는 세션 스토리지는 초기화 될 가능성이 있고 여러 페이지를 전환 할때 마지막 저장 된 값만 활용 하기 때문에 
여러 페이지를 뒤로 가기 하거나 앞으로 가기 할 경우 그 때 그 때 스크롤 위치를 이동 시키기 어려웠습니다.  
이를 해결하기위해 검색을 통해 history.pushState()를 알게 되었고 상품 페이지를 가기 전에 history를 한번 기록하고 넘어가며 보고 있던 정보들을 state를 활용하여 기록 하게 되면서 해결하게 되었습니다.  
...
앞으로 가기로 history 페이지 호출 시 onpopstate 이벤트에서 상품 호출을 하지 않기 때문에 뒤로 가기처럼 원하는 상품 노출을 시켜 줄 수 없었습니다.  
