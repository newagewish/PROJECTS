...
anchor를 사용하여 뒤로 가기 및 앞으로 가기로 페이지 호출 시 자동 스크롤 이동 / 21-04-30 업데이트
...
21-04-30 업데이트  
~~앵커를 이용하여 스크롤 위치를 이동하게끔은 가능해졌지만 페이징 처리를 세션 스토리지를 사용하지 않고  
뒤로 가기, 앞으로 가기로 페이지 호출 시 세션 스토리지가 초기화되더라도 이동 전 상품 개수만큼 데이터를 가져오도록 해야 합니다.~~

21-05-01 업데이트  
세션 스토리지를 사용하지 않고 pushstate를 사용하여 페이지 이동 전 세션 값을 history.pushState로 state에 저장하는 방법을 사용  
뒤로 가기로 해시가 붙은 페이지 호출 시 이전 페이지에 불러왔던 상품 개수와 스크롤 위치 등을 가져오도록 하였고  
그 결과 메인 페이지로 이동 하여 세션 스토리지가 초기화되어도 뒤로 가기 혹은 앞으로 가기로 페이지 진입 시  
이전에 마지막으로 기록된 위치에 접근이 가능해졌다.  

+메인 페이지에는 소메뉴 별로 스크롤 위치를 기억하고 메뉴 이동 시 이전 스크롤 위치로 자동으로 이동하는 기능을 가지고 있는데  
pushstate 이벤트와 겹치게 되면 뒤로 가기 일 때는 그나마 잘 작동하지만 앞으로 가기로 페이지 진입 시 몇 가지 문제점이 발생   
페이징 처리가 될 때마다 history 누적 등을 고려 해보았지만 웹에서는 적합하지 않다는 생각을 했고  
이 시스템은 페이지 전환이 많이 없는 한 페이지 내에서 비동기 통신이 주로 돌아가는 구조에 더 적합한 것 같다는 생각합니다.  
